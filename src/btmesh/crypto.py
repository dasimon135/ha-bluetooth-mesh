...
class K2Output(NamedTuple):
    """Network key material from k2 (spec §3.8.2.6).

    Positionally compatible with the historical ``(nid, enc, priv)`` tuple, so
    ``nid, enc, priv = k2(...)`` keeps working alongside attribute access.
    """
...
def k2(n: bytes, p: bytes) -> K2Output:
    ...
    Returns :class:`K2Output` (NID, EncryptionKey, PrivacyKey); still unpacks
    positionally as ``nid, enc, priv = k2(...)``.
    """
    salt = s1(b"smk2")
    t = aes_cmac(salt, n)
